...
func CreateTenantWorker(b *benchmark.Benchmark, c *engine.DBConnector, testDesc *engine.TestDesc, batch int) (loops int) { //nolint:revive
	...
	if txErr := session.Transact(func(tx db.DatabaseAccessor) error {
		for i := 0; i < batch; i++ {
			var tenantUUID, err = b.Vault.(*engine.DBTestData).TenantsCache.CreateTenant(b.Randomizer.GetWorker(c.WorkerID), tx)
			if err != nil {
				return err
			}

			if b.TestOpts.(*engine.TestOpts).BenchOpts.Events {
				if err = b.Vault.(*engine.DBTestData).EventBus.InsertEvent(b.Randomizer.GetWorker(c.WorkerID), tx, string(tenantUUID)); err != nil {
					return err
				}
			}
		}
...
func CreateCTIEntityWorker(b *benchmark.Benchmark, c *engine.DBConnector, testDesc *engine.TestDesc, batch int) (loops int) { //nolint:revive
	...
	if txErr := session.Transact(func(tx db.DatabaseAccessor) error {
		for i := 0; i < batch; i++ {
			if err := b.Vault.(*engine.DBTestData).TenantsCache.CreateCTIEntity(b.Randomizer.GetWorker(c.WorkerID), tx); err != nil {
				return err
			}
		}
